...
public class Group1_AS extends AcceptanceStrategy {
	...
	/**
	 * Determines whether the last bid from the opponent is acceptable.
	 * The bid is deemed acceptable if the bid is larger than this agent's next bid, or when the deadline has passed and the opponent's last bid is higher than expected.
	 *
	 * @return Actions.Accept if the opponent's bid is acceptable, else Actions.Reject.
	 */
	@Override
	...
	/**
	 * Determines whether a * U_opp + b >= U_next.
	 * U_opp is the utility of the opponents last bid,
	 * U_next is the utility of the agent's next bid.
	 *
	...
	/**
	 * Returns the discounted utility of the best bid of the opponent in the time window (2*t - 1, t] when t > 0.5, else returns 1.
	 *
	 * @return 1 if t < 0.5, discounted utility of opp's best bid in time window (2*t - 1, t].
	 */
	double getMaxW() {
		// FIXME Remove debug output
